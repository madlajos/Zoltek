...
def home_turntable_with_image(image):
    ...
    templateS_name = 'TempHome5.jpg'
    templateC_name = 'TempCenter.jpg'
    templateE_name = 'TEMP_E.jpg'
    best_Type3 = 0
    scale_percent = 5
    ...
    if emsg is not None:
        save_image(last_valid_image, 'E20')
        return None, emsg
    templateE, emsg = load_template(templateE_name, 'E20')
    if emsg is not None:
        save_image(last_valid_image, 'E20')
        return None, emsg
    ...
    if emsg is not None:
        save_image(last_valid_image, 'E20')
        return None, emsg
    templateE_rescaled, emsg = preprocess(templateE, scale_percent)
    if emsg is not None:
        save_image(last_valid_image, 'E20')
        return None, emsg
    ...
    if emsg is not None:
        save_image(last_valid_image, 'E20')
        return None, emsg
    templateS_rescaled, emsg = preprocess(templateS, scale_percent)
    if emsg is not None:
        save_image(last_valid_image, 'E20')
        return None, emsg
    ...
    if emsg is not None:
        ...
        return None, emsg
    # Match template
    best_angle1, best_rotation1, best_Type1, emsg = start_temp_match(templateL_rescaled, templateS, image_rescaled,
                                                                   scale_percent)
    print('BEST TYPE1: ' + str(best_Type1))
    best_angle2, best_rotation2, best_Type2, emsg = start_temp_match(templateE_rescaled, templateS, image_rescaled,
                                                                   scale_percent)
    print('BEST TYPE2: ' + str(best_Type2))
    print(emsg)
    if (best_Type2 < 0.15 or best_Type1 < 0.15) and emsg is None:
        save_image(last_valid_image, 'E24')
        #logger.error("E2400")
        return None, emsg

    if best_Type1+0.05 > best_Type2:
        # Match template
        best_angle, best_rotation, best_Type = best_angle1, best_rotation1, best_Type1
        print("USED TEMPLATE: Dotted")
    else:
        best_angle, best_rotation, best_Type = best_angle2, best_rotation2, best_Type2
        print(best_Type)
        print("USED TEMPLATE: Else")
    if best_Type < 0.25:
        emsg = "2400"
        if emsg is not None:
            save_image(last_valid_image, 'E24')
    ...
    new, result, hit, emsg = fill_second_two_thirds(image, templateC, normalized_angle)
    if emsg is not None:
        save_image(last_valid_image, 'E25')
        # logger.error("E2400")
        return None, emsg
    if best_Type > 0.25 and hit > 0:
        # Match template
        best_angle, best_rotation, best_Type, emsg = start_temp_match(templateS_rescaled, templateS_rescaled, image_rescaled,
                                                                         scale_percent)
        # Determine initial angle
        final_angle = (best_angle + best_rotation) % 360  # Normalize angle to [0, 360)
        normalized_angle = ((final_angle + 180) % 360) - 180  # Converts to [-180, 180]
        if abs(normalized_angle) > 100:
            ...
            adjusted_angle = normalized_angle
        print(best_Type)
        print("USED TEMPLATE: LARGE DOTS")
        new, result, hit, emsg = fill_second_two_thirds(image, templateC, normalized_angle)
    ...
    if hit > 0:
        ...
        # logger.error("E2400")
        return None, emsg
    # cv2.imshow("RotatedImage", cv2.resize(new, None, fx=0.2, fy=0.2, interpolation=cv2.INTER_AREA))
    # cv2.waitKey(0)
    ...
    print('Home process finished successfully.\n''Adjusted angle is ' + str(
        0 if abs(adjusted_angle) <= 0.51 else adjusted_angle))
    # **Apply the rotation**
    # (h, w) = image.shape[:2]
    # center = (w // 2, h // 2)
